...
from .fcfe_modules import FSM, MGSE, SEM
import numpy as np
...
class build_FCFE(nn.Module):
    def __init__(self, num_classes, block=4, M=32, return_f=False, resnet=False, use_fcfe=False,
                 use_normals=False, fcfe_dual_backbone=False, pretrained=True):
        super(build_FCFE, self).__init__()
        self.return_f = return_f
        self.use_fcfe = use_fcfe
        self.use_normals = use_normals
        self.fcfe_dual_backbone = fcfe_dual_backbone
        if resnet:
            resnet_name = "resnet50"
            print('using model_type: {} as a backbone'.format(resnet_name))
            self.in_planes = 2048
            self.backbone = Resnet(pretrained=pretrained)
            self.normal_backbone = Resnet(pretrained=pretrained) if use_fcfe and use_normals and fcfe_dual_backbone else self.backbone
        else:
            ...
            if 'base' in convnext_name:
                ...
                self.in_planes = 768
            self.backbone = create_model(convnext_name, pretrained=pretrained)
            if use_fcfe and use_normals and fcfe_dual_backbone:
                self.normal_backbone = create_model(convnext_name, pretrained=pretrained)
            else:
                self.normal_backbone = self.backbone

        self.num_classes = num_classes
        self.block = block
        self.M = M
        if self.use_fcfe:
            self.MGSE_layer = MGSE(self.in_planes)
            self.SEM_layer = SEM(self.in_planes)
            self.FSM_layer = FSM(self.in_planes)
    ...
    def forward(self, x, normal=None):
        ...
        if self.use_fcfe:
            if normal is not None:
                _, normal_features = self.normal_backbone(normal)
                part_features = self.MGSE_layer(part_features, normal_features)
            part_features = self.SEM_layer(part_features)
            part_features = self.FSM_layer(part_features)
            if hasattr(self.backbone, 'norm'):
                gap_feature = self.backbone.norm(part_features.mean([-2, -1]))
            else:
...
def make_FCFE_model(num_class, block=4, M=32, return_f=False, resnet=False, use_fcfe=False,
                    use_normals=False, fcfe_dual_backbone=False, pretrained=True):
    print('===========building convnext===========')
    model = build_FCFE(num_class, block=block, M=M, return_f=return_f, resnet=resnet, use_fcfe=use_fcfe,
                       use_normals=use_normals, fcfe_dual_backbone=fcfe_dual_backbone, pretrained=pretrained)
    return model
