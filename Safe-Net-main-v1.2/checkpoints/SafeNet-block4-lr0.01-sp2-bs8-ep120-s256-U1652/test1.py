...
from PIL import Image  # 新增：用于加载单张图像
...
parser.add_argument('--img_path', type=str, default='/opt/Safe-Net-main3/checkpoints/SafeNet-block4-lr0.01-sp2-bs8-ep120-s256-U1652/img/1.jpeg', help='输入图像路径，用于生成热力图')
...
######################################################################
# 图像预处理（与测试集保持一致，确保特征匹配）
# ---------
def get_transforms():
    """获取与测试集一致的图像预处理流程"""
    # 根据模式选择预处理（query可能有pad，gallery没有）
    if opt.mode == 1:
        # mode=1: query是satellite（用query_transforms）
        ...
        # mode=2: query是drone（用query_transforms）
        transform = transforms.Compose([
            transforms.Resize((opt.h, opt.w), interpolation=3),
            Query_transforms(pad=opt.pad, size=opt.w),
            transforms.ToTensor(),
...
print(f"使用模型: {opt.checkpoint}")
model = model.eval()  # 评估模式
if torch.cuda.is_available():
    model = model.cuda()

######################################################################
# 处理单张图像热力图（核心修改：优先执行热力图生成）
# ---------
if opt.img_path:
    # 检查图像像路径是否存在
    ...
    # 加载并预处理图像
    img = Image.open(opt.img_path).convert('RGB')  # 加载图像
    transform = get_transforms()  # 获取预处理函数
    img_tensor = transform(img).unsqueeze(0)  # 增加batch维度
    
    # 生成热力图
    print(f"开始生成热力图: {opt.img_path}")
    generate_heatmap(model, opt.img_path, opt)  # 注意：这里传入预处理后的tensor
    print("热力图生成完成！")
    exit()  # 生成后直接退出，不执行测试集流程

######################################################################
# 以下为原测试集处理流程（当不指定--img_path时执行）
# ---------
######################################################################
# Load Data
# ---------
