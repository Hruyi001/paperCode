...
######################################################################
def load_network(opt, gpu_ids):
    save_filename = opt.checkpoint

    # 检查是否有save_intermediate参数
    save_intermediate = getattr(opt, 'save_intermediate', False)
    model = Safe_Net(opt.nclasses, block=opt.block, imgsize=opt.h, save_intermediate=save_intermediate)
