...
class Safe_Net_model(nn.Module):
    def __init__(self, num_classes, block = 4 ,return_f=False, imgsize=256, save_intermediate=False):
    ...
    def forward(self, x):
        ...
        if self.save_intermediate:
            ...
            # 获取boundaries（分区边界）
            boundaries = self.get_boundary(aligned_feature_map)
            # 保存中间输出
            self.intermediate_outputs = {
                "theta": A_theta.detach().cpu(),  # [B, 2, 3]
                "boundaries": boundaries,  # list of lists
                "f_p_aligned": aligned_feature_map.detach().cpu()  # [B, C, H, W]
            }
