...
try:
    ...
    def _warmup_factor(current_step, num_warmup_steps):
        if num_warmup_steps <= 0 or current_step >= num_warmup_steps:
            return 1.0
        return float(current_step) / float(max(1, num_warmup_steps))

    def get_constant_schedule_with_warmup(optimizer, num_warmup_steps):
        return LambdaLR(optimizer, lambda step: _warmup_factor(step, num_warmup_steps))
    ...
    def get_polynomial_decay_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps, lr_end=1e-7, power=1.0):
        ...
        def lr_lambda(step):
            if step < num_warmup_steps:
                return _warmup_factor(step, num_warmup_steps)
            progress = float(step - num_warmup_steps) / float(max(1, num_training_steps - num_warmup_steps))
            decay = (lr_init - lr_end) * (1.0 - progress) ** power + lr_end
...
from sample4geo.loss.alignment_loss import AlignmentLoss
...
class Configuration:
    def __init__(self):
        ...
        parser.add_argument('--handcraft_model', default=True, type=str2bool, help='use modified backbone')
        ...
        parser.add_argument('--record', default=True, type=str2bool, help='use tensorboard to record training procedure')

        parser.add_argument('--only_test', default=False, type=str2bool, help='use pretrained model to test')
        ...
        parser.add_argument('--use_vrm', default=True, type=str2bool, help='enable semantic-guided view rectification module')
        parser.add_argument('--use_sam', default=True, type=str2bool, help='enable scale-adaptive feature calibration module')
        parser.add_argument('--use_csm', default=True, type=str2bool, help='enable cross-view semantic alignment module')
        parser.add_argument('--vcsa_residual_scale', default=0.1, type=float, help='initial residual scale for VRM/SAM/CSM modules')
        ...
        parser.add_argument('--weight_alignment', default=0.6, type=float)

        # Training Config
        parser.add_argument('--mixed_precision', default=True, type=str2bool)
        parser.add_argument('--custom_sampling', default=True, type=str2bool)
        parser.add_argument('--seed', default=1, type=int, help='random seed')
        parser.add_argument('--epochs', default=1, type=int, help='1 epoch for 1652')
        parser.add_argument('--max_train_steps', default=None, type=int, help='limit training steps per epoch for debugging')
        ...
        parser.add_argument('--normalize_features', default=True, type=str2bool)
        parser.add_argument('--eval_gallery_n', default=-1, type=int)

        # Optimizer Config
        parser.add_argument('--clip_grad', default=100.0, type=float)
        parser.add_argument('--decay_exclue_bias', default=False, type=str2bool)
        parser.add_argument('--grad_checkpointing', default=False, type=str2bool)
        ...
        parser.add_argument('--zero_shot', default=False, type=str2bool)
        ...
        parser.add_argument('--cudnn_benchmark', default=True, type=str2bool)

        # Make cudnn deterministic Config
        parser.add_argument('--cudnn_deterministic', default=False, type=str2bool)
...
if __name__ == '__main__':
    ...
    if config.handcraft_model is not True:
        ...
        if hasattr(model, "configure_vcsa_modules"):
            model.configure_vcsa_modules(
                use_vrm=config.use_vrm,
                use_sam=config.use_sam,
                use_csm=config.use_csm,
                residual_scale=config.vcsa_residual_scale,
            )
            print(f"VCSA modules: VRM={config.use_vrm}, SAM={config.use_sam}, CSM={config.use_csm}, residual_scale={config.vcsa_residual_scale}")

    # -- print weight config infos
    print(
        f"\nweight_infonce:{config.weight_infonce}\nconfig.weight_gcc:{config.weight_cls}\nweight_alignment:{config.weight_alignment}\n")
    ...
    loss_fn4 = AlignmentLoss(loss_function=loss_fn, device=config.device)

    loss_functions = {"infoNCE": loss_fn1, "Triplet": loss_fn2, "blocks_infoNCE": loss_fn3, "alignment_loss": loss_fn4}
