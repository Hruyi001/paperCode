    ...
    for query, reference, ids, labels in bar:
        ...
        if scaler:
            with (autocast()):  # -- 使用混合精度
                ...
                if train_config.handcraft_model is not True:
                    ...
                    features_alignment_1, features_alignment_2 = output1[0], output2[0]

                if torch.cuda.device_count() > 1 and len(train_config.gpu_ids) > 1:
                    logit_scale = model.module.logit_scale.exp()
                    logit_scale_blocks = model.module.logit_scale_blocks.exp()
                else:
                    logit_scale = model.logit_scale.exp()
                    logit_scale_blocks = model.logit_scale_blocks.exp()

                loss = loss_functions["infoNCE"](features1, features2, logit_scale)
                loss_cls = cal_loss(features_cls_1, labels, criterion) + cal_loss(features_cls_2, labels, criterion)
                loss_alignment = loss_functions["alignment_loss"](features_alignment_1, features_alignment_2, logit_scale_blocks)

                lossall = train_config.weight_infonce * loss + train_config.weight_cls * loss_cls + train_config.weight_alignment * loss_alignment

                # lossall = 1.0 * loss + 0.0 * loss_cls + 0.0 * loss_alignment

                if not torch.isfinite(lossall):
                    optimizer.zero_grad(set_to_none=True)
                    step += 1
                    continue
        ...
        if train_config.verbose:
            ...
            monitor = {
                ...
                "loss_alignment": "{:.4f}".format(train_config.weight_alignment * loss_alignment.item()),
