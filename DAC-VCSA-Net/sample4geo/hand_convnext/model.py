...
class two_view_net(nn.Module):
    ...
    def configure_vcsa_modules(self, use_vrm=True, use_sam=True, use_csm=True, residual_scale=0.1):
        if hasattr(self.model_1, "configure_vcsa_modules"):
            self.model_1.configure_vcsa_modules(
                use_vrm=use_vrm,
                use_sam=use_sam,
                use_csm=use_csm,
    ...
    def forward(self, x1, x2=None):
        ...
        if x2 is not None:
            if hasattr(self.model_1, "forward_pair"):
                y1, y2 = self.model_1.forward_pair(x1, x2)
            else:
                y1 = self.model_1(x1)
                y2 = self.model_1(x2)
            return y1, y2
