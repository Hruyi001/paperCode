...
def load_pretrained_checkpoint(url):
    checkpoint_path = os.path.join(torch.hub.get_dir(), "checkpoints", os.path.basename(url))
...
def convnext_tiny(pretrained=True, in_22k=True, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = load_pretrained_checkpoint(url)
        print(url)
