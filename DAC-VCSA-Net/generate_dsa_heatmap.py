...
class ModelWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        # Both w/ and w/o DSA use the same proj layer to generate attention weights
        # This ensures consistent extraction method
        W = self.model.model_1.proj(pfeat)  # (bs, 256, h*w)
        
        if self.use_dsa:
            # With DSA: use the trained DSA attention mechanism
            ...
            # Without DSA: use the same proj layer but with different normalization
            # This ensures the extraction method is similar, making comparison fair
            # Use sigmoid instead of softmax to get attention-like weights without strict normalization
            W = F.normalize(W, dim=1) if self.model.model_1.l2_norm else W
            W = torch.sigmoid(W)  # Use sigmoid to get attention-like weights (0-1 range)
            
            # Reshape W to spatial dimensions
            W_spatial = W.view(b, -1, h, w)
            
            # Aggregate across channels (same as w/ DSA)
            attention_map = W_spatial.mean(dim=1)  # (bs, h, w)
            
            # Optional: apply slight smoothing to match w/ DSA's distribution
            # This helps make the comparison more fair
            kernel_size = 3
