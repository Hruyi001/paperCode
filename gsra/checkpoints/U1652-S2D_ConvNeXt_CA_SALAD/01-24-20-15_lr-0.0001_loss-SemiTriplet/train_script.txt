...
# -*- coding: utf-8 -*-
"""
Training script for comparative attention methods (baseline, CBAM, CA, ECA, EMA).
This script can be used to train models with different attention mechanisms for ablation studies.
"""
...
class Configuration:
    ...
    attention: str = None  # None for baseline, or 'CBAM', 'CA', 'ECA', 'EMA', 'GSRA'
    ...
    data_folder: str = "/root/dataset/University-Release"
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train comparative attention methods')
    parser.add_argument('--attention', type=str, default=None,
                        choices=['baseline', 'CBAM', 'CA', 'ECA', 'EMA', 'GSRA', None],
                        help='Attention mechanism to use. "baseline" means no attention (None)')
    parser.add_argument('--dataset', type=str, default='U1652-D2S',
                        choices=['U1652-D2S', 'U1652-S2D'],
                        help='Dataset configuration')
    parser.add_argument('--data_folder', type=str, default='/root/dataset/University-Release',
    ...
    parser.add_argument('--model_path', type=str, default='./checkpoints',
                        help='Base path for saving checkpoints')
...
def main():
    ...
    # Map 'baseline' to None for attention
    attention = None if args.attention == 'baseline' else args.attention
    ...
    config.lr = args.lr
    config.gpu_ids = tuple(args.gpu_ids)
    ...
    # Create model path with attention name
    attn_name = 'baseline' if attention is None else attention
    model_path = "{}/{}_{}_{}_{}/{}_lr-{}_loss-{}".format(
        config.model_path, config.dataset, config.backbone,
        attn_name, config.aggregation,
        time.strftime('%m-%d-%H-%M'), config.lr, config.loss
    )
    
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    shutil.copyfile(os.path.basename(__file__), "{}/train_script.txt".format(model_path))
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if r1_test > best_score:
            if torch.cuda.device_count() > 1 and len(config.gpu_ids) > 1:
                torch.save(model.module.state_dict(), '{}/best_score.pth'.format(model_path))
            else:
                torch.save(model.state_dict(), '{}/best_score.pth'.format(model_path))
            best_score = r1_test
    ...
    print(f"Best checkpoint saved at: {model_path}/best_score.pth")
